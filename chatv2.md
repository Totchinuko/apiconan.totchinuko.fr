...
```csharp
// Server
DestroyChannel(out bool Success, out string ErrorMessage);
```
...
```csharp
// Server
GetChannelUID(out long ReturnValue);
```
...
```csharp
// Server
SendMessage(Guid Guid, long Sender, Tot_S_ChatHeader Headers, string Content);
```
...
```csharp
// Server
SendTSignal(long Sender, Tot_S_ChatHeader Headers);
```
...
```csharp
// Server
ConnectUser(Tot_I_ChatUser user, string Password, bool ForceSwitch);
```
...
```csharp
// Server
DisconnectUser(Tot_I_ChatUser user);
```
Disconnect a user from the channel, removing it from the user Channel List.

```csharp
// Server
GetChannelName(out string ReturnValue);
```
Display name of the channel for Admin sections

```csharp
// Server
GetPersonalizedChannelName(Tot_I_ChatUser user, out string ReturnValue);
```
...
```csharp
// Server
GetConnectUsers(out Tot_I_ChatUser[] ReturnValue);
```
List all the users connected to this channel

```csharp
// Server
GetChannelType(out Name ReturnValue);
```
...
```csharp
// Server
BindObserver(Tot_I_ChatChannelObserver Observer);
```
...
```csharp
// Server
UnbindObserver(Tot_I_ChatChannelObserver Observer);
```
...
```csharp
// Server
MessageProcessed(Guid Guid, long Sender, long Channel, Tot_S_ChatHeader Headers, string Content);
```
...
```csharp
// Server
ExecuteCommand(Name CommandRow, long sender, long channel, Tot_S_ChatHeader Headers, string[] Arguments);
```
...
```csharp
// Server
HandleCommand(Name Command, Name SubCommand, long Sender, long Channel, Tot_S_ChatHeader Headers, string[] Arguments, out bool Handled);
```
...
```csharp
// Server
GetEntitySettings(out Tot_S_ChatHeader ReturnValue);
```
Get the entity settings. For detail about the Chat Headers, see the dedicated section.

```csharp
// Server
SetEntitySettings(Tot_S_ChatHeader Settings);
```
...
```csharp
// Server
GetEntityType(out Name ReturnValue);
```
...
```csharp
// Server
GetEntityName(out string ReturnValue);
```
Display name of the entity, mostly for admin panel purposes.

```csharp
// Server
GetEntityUID(out long ReturnValue);
```
...
```csharp
// Client
SetSettings(long EntityUID, Name EntityType, string EntityName, Tot_S_ChatHeader Settings);
```
This will be called on your form to pass the data of the edited entity.

```csharp
// Client
GetSettings(out Tot_S_ChatHeader Settings);
```
...
```csharp
// Server/Client
GetRangesLength(out int ReturnValue);
```
Length of the Range List

```csharp
// Server/Client
GetRangeAt(int index, out Guid guid, out string Name, out Name Command, out float Range, out Color Color);
```
Get the data of a range at a given index.

```csharp
// Server/Client
GetRange(Guid guid, out int index, out string Name, out Name Command, out float Range, out Color Color);
```
Get the data of a range corresponding to a given index. Index is -1 if the Guid was not found.

```csharp
// Server/Client
GetDefaultRange(out Guid Guid);
```
Get the default range (Which is always the first of the list)

```csharp
// Server/Client
GetLanguageLength(out int ReturnValue);
```
Get the length of the Language List

```csharp
// Server/Client
GetLanguageAt(int index, out Guid Guid, out string Name, out Name Font, out bool Admin Only, out bool Universal, out float StartingKnowledge, out int LearningFraction, out float LearningPower);
```
...
```csharp
// Server/Client
GetLanguageAt(Guid Guid, out int Index, out string Name, out Name Font, out bool Admin Only, out bool Universal, out float StartingKnowledge, out int LearningFraction, out float LearningPower);
```
Get the data of a Language for a given Guid. Index is -1 if the Guid was not found.

```csharp
// Server
ExecuteCommand(long sender, long channel, Name CommandRow, Tot_S_ChatHeader Headers, string[] Args);
```
Should not be called directly. Command execution should be called from the User, or some of the code will be skipped. 

```csharp
// Server
GetMainChannels(out Tot_I_ChatChannel MainGlobal, out Tot_I_ChatChannel MainLocal);
```
...
```csharp
// Server
GetGuildChannelOrCreate(Unique_ID GuildUniqueID, out Tot_I_ChatChannel ReturnValue);
```
...
```csharp
// Server
PushChannelCache(Guid guid, long Sender, long Channel, Tot_S_ChatHeader headers, string Content);
```
...
```csharp
// Server
PurgeChannelCache(long Channel);
```
...
```csharp
// Client
ParseText(string Text, Tot_S_ChatHeader Headers, int LanguageSeed, float LanguageKnowledge, bool RemoveEmotes, out string ReturnValue, int[] InsertedItems, Name LanguageFont);
```
...
```csharp
// Client
SpawnUserBubble(Tot_I_ChatUser User, out Tot_I_ChatUserBubble ReturnValue);
```
...
GetUserUID(out int ReturnValue);
```
Unique_ID of the user, directly converted from the persistency.

```csharp
// Server/Client(For BPC only)
GetUserName(out string ReturnValue);
```
...
```csharp
// Server
SendMessageAsUser(Guig guid, long Channel, Tot_S_ChatHeader Headers, string Content);
```
Send a message as if sent by the user. Do not send commands through there, it's already too late in the chain.

```csharp
// Server
BindObserver(Tot_I_ChatUserObserver Observer);
```
...
```csharp
// Client
MessageReceived(Guid Guid, long Sender, long Channel, long Recipient, Tot_S_ChatHeader headers, string Content);
```
...
```csharp
// Client
TSignalReceived(long Sender, long Channel, long Recipient, Tot_S_ChatHeader headers);
```
...
```csharp
// Server
UserReceivedMessage(Guid Guid, long Sender, long Channel, long Recipient, Tot_S_ChatHeader headers, string Content);
```
Called whenever a bound user receive a message. This includes edition of past messages.

```csharp
// Server
UserSentMessage(Guid Guid, long Sender, long Channel, Tot_S_ChatHeader headers, string Content);
```
